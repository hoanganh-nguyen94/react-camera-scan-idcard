...
public class CropperFrameProcessorPlugin: FrameProcessorPlugin {
  ...
  public override func callback(
    ...
    if cropRegion != nil {
      ...
      let left:Double = Double(
        cropRegion?["left"] ?? 0
      ) / 100.0 * imgWidth
      let top:Double = Double(
        cropRegion?["top"] ?? 0
      ) / 100.0 * imgHeight
    ...
    if includeImageBase64 == true {
      cropResult["base64"] = getBase64FromImage(
        image
      )
    }
